...
<div class="main-content">
    <div class="row">
        ...
        <div class="col-md-6 col-sm-8 clearfix">
            ...
            if (isset($user_infos[0]['user_id']) && $user_infos[0]['user_id'] != '') {
                ?>
                <h5>
                    <a href="<?php echo base_url();?>user/view_message/<?php echo $user_infos[0]['user_id']; ?>"
                       class="btn btn-green btn-icon btn-xs" title="See all messages of this user">
                        <i class="entypo-mail"></i>View All Messages
                    </a>
                    <?php if ($user_infos[0]['user_type'] != '1') { ?>
                        <a href="<?php echo base_url(); ?>property/probyuser-<?php echo $user_infos[0]['user_id']; ?>/all"
                           title="See all properties of this user" class="btn btn-black btn-icon btn-xs">
                            <i class="fa fa-archive"></i>View All Properties
                        </a>
                    <?php } ?>
                    <a href="<?php echo base_url();?>user/edit_profile/<?php echo $user_infos[0]['user_id']; ?>"
                       style="width:120px;" class="btn btn-default editbtn btn-sm btn-icon btn-xs"
	...
	<span class="filterTbl">
		<a <?php echo($activetype == 'all' ? 'class="selected"' : ''); ?>
            href="<?php echo base_url() . 'property/' . ($type == '' ? 'all' : $type) . '/all' . ($search_property_code != '' ? '?search_property_code=' . $search_property_code : ''); ?>">
            All (<?php echo $allFilterProperty; ?>)
        </a>
		-
		<a <?php echo($activetype == 'activepro' ? 'class="selected"' : ''); ?>
            href="<?php echo base_url() . 'property/' . ($type == '' ? 'all' : $type) . '/activepro' . ($search_property_code != '' ? '?search_property_code=' . $search_property_code : ''); ?>">
            Published (<?php echo $activeFilterProperty; ?>)
        </a>
		-
		<a <?php echo($activetype == 'inactivepro' ? 'class="selected"' : ''); ?>
            href="<?php echo base_url() . 'property/' . ($type == '' ? 'all' : $type) . '/inactivepro' . ($search_property_code != '' ? '?search_property_code=' . $search_property_code : ''); ?>">
            InActive (<?php echo $inActiveFilterProperty; ?>)
        </a>
		-
		<a <?php echo($activetype == 'featured' ? 'class="selected"' : ''); ?>
            href="<?php echo base_url() . 'property/' . ($type == '' ? 'all' : $type) . '/featured' . ($search_property_code != '' ? '?search_property_code=' . $search_property_code : ''); ?>">
            Featured (<?php echo $featuredFilterProperty; ?>)
        </a>
		-
		<a <?php echo($activetype == 'suspended' ? 'class="selected"' : ''); ?>
            href="<?php echo base_url() . 'property/' . ($type == '' ? 'all' : $type) . '/suspended' . ($search_property_code != '' ? '?search_property_code=' . $search_property_code : ''); ?>">
	...
	<span id="search" style="float:right;margin-bottom:15px;">
		<form method="get" action="<?php echo base_url(); ?>property/all/all">
            <input type="text" name="search_property_code" class="search-input" placeholder="Search by property ID-Code"
                   style="width:400px" value="<?php echo $search_property_code; ?>">
            <button type="submit">
        ...
        if (!empty($property_details)) {
            ...
            foreach ($property_details as $property_detail) {
                ...
                <tr>
                    ...
                    <td>
                        <!--
					<a class="btn btn-default editbtn btn-sm btn-icon btn-xs" title="Click here to view" href="<?php //echo base_url();
                        ?>property/view_property_details/<?php //echo $property_detail['property_id'];
                        ?>">
						<i class="entypo-search"></i>&nbsp;
					</a>
					-->
                        <a class="btn btn-default editbtn btn-sm btn-icon btn-xs" title="Click here to edit"
                           href="<?php echo base_url();?>property/edit_property_details/<?php echo $property_detail['property_id']; ?>">
                        ...
                        ?>
                        <a <?php echo $statusURL;?>
                            class="btn btn-success <?php echo $btn_class; ?> btn-sm btn-icon btn-xs">
                        ...
                        echo $featuredLink;
                        ?>
                        <a href="<?php echo base_url();?>property/delete_property/<?php echo $property_detail['property_id'] . $pageLink; ?>"
