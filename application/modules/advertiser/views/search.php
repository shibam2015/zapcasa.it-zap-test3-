	...
	<div class="searchresult_box searchresult_box2" style="height:660px ! important;overflow:hidden;">
		...
		?>
						...
						<div class="propFeatures">
							...
							<h3><?php echo $this->lang->line('advertise_list_property_post');?> <font style="color:#ED6B1F"><?php echo get_perticular_count('zc_property_details'," and property_post_by='".$arrAdv->user_id."' AND property_status='2' AND suspention_status='0' AND property_approval ='1'");?></font></h3>
...
//echo "=================== > ".$ShowingCityName;
				...
				$advertiserGeoAddress1 = ($arrAdv->area!=''?$arrAdv->area.' - ':'');
                                            $advertiserGeoAddress1.= ($arrAdv->street_address!=''?$arrAdv->street_address.', ':'');
                                            $advertiserGeoAddress1.= ($arrAdv->street_no!=''?$arrAdv->street_no.' - ':'');
                                            $advertiserGeoAddress1.= ($arrAdv->zip!=''?$arrAdv->zip.'  ':'');
                                            $advertiserGeoAddress1.= stripslashes($ShowingCityName).' - ';
                                            $advertiserGeoAddress1.= $st_name1;
                                             //$advertiserGeoAddress1.= ($arrAdv->zip!=''?$arrAdv->zip:'');
                                          //  echo $advertiserGeoAddress1;

				$advertiserGeoAddress= stripslashes($arrAdv->city).', '.$arrAdv->province.', '.$arrAdv->zip.', Italy';
				//$advertiserGeoAddress1= stripslashes($arrAdv->city).', '.$arrAdv->province.', '.$arrAdv->zip.', Italy';
				  // echo "<pre>";
			   // print_r($arrAdv);exit;
				...
				$GoogleMapMarkers[$gMapCounter] = array(  
					...
					'proaddress' => $advertiserGeoAddress1,
