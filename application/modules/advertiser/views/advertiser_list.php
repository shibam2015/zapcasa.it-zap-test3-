...
<div class="main main_searchpage">
	<div id="breadcrumb" class="fk-lbreadbcrumb newvd">
    	<span><a href="<?php echo base_url();?>"><?php echo $this->lang->line('advertise_list_home');?></a></span> > <span><a href="<?php echo base_url()."advertiser/search?location=&name=&advertiser_type=all";?>"><?php echo $this->lang->line('advertiser_title_details');?></a></span> >
	        	...
	        	<li>
	            	<div class="listingImg">
						...
						?>
	                     ...
	                     ?>						 <div class="listingShw" style="background-size:105px;"></div>
	                    </a>
	                </div>
					...
					<h2 style="padding: 0 0 5px;" ><a href="<?php echo $link;?>"><?php echo $business_name;?></a></h2>
	                    <?php if( $user_pref[0]['my_address_display'] == 0 ) {  ?>
		                    <div class="listAddress">
								...
								<h4>
									<?php																		if(!strpos($advertiser_list['province'], "'")===false){										$st_name1=get_perticular_field_value('zc_region_master','province_code'," and `province_name` = '".str_replace("'","\\\''",stripslashes($advertiser_list['province']))."' group by province_code");									}else{										$st_name1=get_perticular_field_value('zc_region_master','province_code'," and `province_name` LIKE '%".$advertiser_list['province']."%' group by province_code");									}																		
							  			if( $advertiser_list['street_address'] ) { echo stripslashes($advertiser_list['street_address']); }
							  			if( $advertiser_list['street_no'] ) { echo ', '.stripslashes($advertiser_list['street_no']); }
							  			if( $advertiser_list['zip'] ) { echo ' - '.$advertiser_list['zip']; }
							  			if($advertiser_list['city']){											if(!strpos($advertiser_list['city'], "'")===false){												$city_name=get_perticular_field_value('zc_city',($_COOKIE['lang']=='english'?"city_name":"city_name_it")," and `city_name` = '".str_replace("'","\\\''",stripslashes($advertiser_list['city']))."' OR `city_name_it` = '".str_replace("'","\\\''",stripslashes($advertiser_list['city']))."'");											}else{												$city_name=get_perticular_field_value('zc_city',($_COOKIE['lang']=='english'?"city_name":"city_name_it")," and `city_name` = '".$advertiser_list['city']."' OR `city_name_it` = '".$advertiser_list['city']."'");											}											echo ' '.stripslashes($city_name);										}
