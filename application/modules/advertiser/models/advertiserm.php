...
class advertiserm extends CI_Model {
	...
	public function getAdvertiserByFilter($filters = '', $startpoint, $perpage, $pagination)
	{
...
//echo "<pre>"; print_r($filters);exit;
	...
	function get_list($filters, $startpoint, $perpage, $location, $name, $advertiser_type, $limit, $start)
	{
	...
	function get_feedback($limit=0, $start=0) {
		...
		$sql = "select * from zc_feedback where feedback_to_id='" . $feedback_to_id . "' and feedback_status='1' ORDER BY feedback_id DESC LIMIT $start ,$limit";
	...
	public function AdvertiserListingnewPages($filters, $limit, $path, $perpage, $cureentPage, $returnType)
	{
		...
		if ($filters['advertiser_type'] != '' && $filters['advertiser_type'] != '3') {


			$where .= " and user_type = 3";
...
//echo "<pre>"; print_r($filters);exit;
		...
		$sql = "select * from zc_user left join zc_user_preference on zc_user.user_id = zc_user_preference.user_id where user_type not in(1,2,4) " . $where . " and zc_user.status != '0' and zc_user.verified != '0' and zc_user_preference.invisible = '0' and zc_user_preference.my_address_display = '0' order by zc_user.user_id desc";
