...
class property extends CI_Controller {
	...
	public function msg_details(){
		...
		if(count($all_msgs) > 0){
			...
			if($all_msgs[0]['property_id']!='0'){
				...
				$subjectLine = ($lang=='it'?'Richiesta per':'Request for').": ".(strlen(subject_inbox($all_msgs[0]['property_id'])) > 11)?subject_inbox($all_msgs[0]['property_id']):$this->lang->line('prop_not_available');	/*$this->lang->line('property_request_for').':'.*/
	...
	public function saved_search_prop(){
		...
		$new_data['property_post_by_type']	= $property_post_by_type;
		...
		if( isset($new_saved_search['save_search_id']) && ($new_saved_search['save_search_id']!= "")){

			$save_search_id=$new_saved_search['save_search_id'];
	...
	public function save_search_modify() {
		...
		if(count($rs) > 0 ){
			...
			$searchPrms = trim(preg_replace('/[stnrs]+/', ' ', $data));
	...
	public function delete_msg_per(){
		...
		if(($msgDetails[0]['msg_from_delete'] == '1' && $msgDetails[0]['msg_to_delete'] == '0')){
			...
			$rs=$this->propertym->delete_per_msg($delete_msg_ids);
