...
<body class="noJS">
            ...
            <div class="inbox_area">
                ...
                <div style="float:left; width:100%; height:40px;">
                    <div class="inbox_tabs">
                        <ul>
                            <li>
								<a class="active" href="<?php echo base_url();?>property/get_message">
									<?php echo $this->lang->line('inbox_inbox_tabs_inbox');?>
								</a>
							</li>
							<li>
								<a href="<?php echo base_url();?>property/get_send_message">
									<?php echo $this->lang->line('inbox_archive');?>
								</a>
							</li>
							<?php
							$inboxSesUid=$this->session->userdata('user_id');
							$inboxSesUserType=get_perticular_field_value('zc_user','user_type'," and user_id='".$inboxSesUid."'");
							if($inboxSesUserType!='1'){
							?>
							<li>
							...
							<li>
								<a href="javascript:void(0);" onClick="return delete_msg();">Delete</a>
							</li>
							-->
                        </ul>
                    </div>
                ...
                <div class="advertiser_details save_search_width">
                    <div class="inbox_delete_pagination" id="inbox_delete_pagination" >
                        <?php
							$perMessage = 10;
                            $msg_tot=count($msg_totals);
							$noOfEntry = $msg_tot;
							if($noOfEntry==1){
                    ...
                    <table width="100%" border="0" cellspacing="3" cellpadding="3"  id="inbox_msg">
                        <tr class="heading">
                            ...
                            <td width="21%"><?php echo $this->lang->line('inbox_inbox_message_from');?></td>
				...
				if(!empty($msg_totals)){
					...
					foreach($msg_totals as $msg){						
						...
						$unReadMsgCounter = get_perticular_count('zc_property_message_info',"and user_id_to = '".$msg['user_id_to']."' and msg_grp_id = '".$msg['msg_grp_id']."' and read_status = '0' and msg_to_delete = '0'")
						?>
                        <tr <?php echo $class;?> >
							...
							if($msg['property_id']!='0'){
                                ...
                                <span <?php echo($unReadMsgCounter > 0?'style="font-weight:bold;cursor:pointer;"':'style="cursor:pointer;"'); ?> onClick="return click_me(<?php echo $msg['msg_id'];?>,<?php echo $msg['property_id'];?>,<?php echo $check_user_to[0]['status'];?>);">
									<?php echo $this->lang->line('inbox_request_for'),":";?> <?php echo subject_inbox($msg['property_id']);?>
								</span>
                                <?php
							}else{
								if($msg['subject'] == 0){
									$noProMsgSub = get_perticular_field_value('zc_property_message_info','subject',"and subject !='0' and msg_grp_id = '".$msg['msg_grp_id']."' and msg_to_delete = '0'");
								}else{
									$noProMsgSub = $msg['subject'];
								}
								?>
                                <span <?php echo($unReadMsgCounter > 0?'style="font-weight:bold;cursor:pointer;"':'style="cursor:pointer;"'); ?> onClick="return click_me(<?php echo $msg['msg_id'];?>,<?php echo $msg['property_id'];?>,<?php echo $check_user_to[0]['status'];?>);">
									<?php //echo $this->lang->line('inbox_subject').":";?><?php echo ucfirst($noProMsgSub);?>
								</span>
                                <?php
							}
							?>
							...
							<?php 
                                ...
                                //echo $this->lang->line('inbox_by').' '.ucfirst($fname." ". $lname);
                        ...
                        <tr id="msg_det_<?php echo $msg['msg_id'];?>" style="display:none; background-color:#3D8AC1" onClick="return close_me(<?php echo $msg['msg_id'];?>);">
                            <td colspan="4">
                                <span style="color:#FFFFFF"><?php echo $this->lang->line('inbox_message_detail');?>: </span><?php echo ucfirst($msg['message']);?><br>
                                <span style="color:#FFFFFF"><?php echo $this->lang->line('inbox_email_id');?>: </span><?php echo $msg['email_id'];?><br>
                                <span style="color:#FFFFFF"><?php echo $this->lang->line('inbox_phone_number');?>: </span><?php echo $msg['ph_number'];?>
                            </td>
                        </tr>
                        <?php
						$i++;
					...
					?>
                        <tr>
                            <td colspan='4' style="text-align:center;"><?php echo $this->lang->line('inbox_sorry_your_inbox_is_empty');?></td>
                        </tr>
					<?php
				}                            
				?>
					...
					<table id="bbb" style="display:none;">
                        ...
                        <div style="display:none;" id="reply_msg">
							...
							?> 
							<!--
							<div class="search-fld" style="width:98% !important; margin-top: 16px;"> 
                                <input type="text" <?php //echo $disable;?> class="required" placeholder="<?php //echo $this->lang->line('inbox_field_subject');?>" name="subject" id="subject" style="width: 100%; height: 25px; resize:none;" />
                            </div>
							-->
							<label id="errorMessage">
								<?php echo($_COOKIE['lang']=='it'?'Campo obbligatorio.':'This field is required.'); ?>
							</label>
                            <div class="search-fld" style="width:98% !important;">								
                                <textarea rows="3" <?php echo $disable;?> class="required" cols="15" placeholder="<?php echo $this->lang->line('contactus_fill_the_form_write_your_message');?>" name="message" id="message" style="width: 100%; height: 164px; resize:none;"></textarea> 
                            </div>
                            <div>&nbsp;</div>
	...
	function delete_msg() {
	   ...
	   if (checkedValues == '') {
		   alert('<?php echo $this->lang->line('
			   inbox_please_select_message_which_you_want_to_delete ');?>');
		   return false;
	   } else {
		   ///////ajax posting/////////////////////
		   $.ajax({
	...
	function click_me(id, prop_id,user_status) {		
		...
		if(user_status=='1'){
			...
			if (prop_id != 0) {
				$("#inbox_delete_pagination").html("<a class='msg_del' href='javascript:void(0);'><img title='<?php echo $this->lang->line('inbox_heading_delete_icon_title');?>' style='cursor:pointer;' onclick='return delete_pmsg(" + id + ");' src='<?php echo base_url();?>assets/images/delete_icon.png'></a><a class='view_prop' href='<?php echo base_url();?>property/view_property/" + id + "'><?php echo $this->lang->line('inbox_message_view_property');?></a>");
				open_reply();
			} else {
				$("#inbox_delete_pagination").html("<a class='msg_del'  href='javascript:void(0);'><img title='<?php echo $this->lang->line('inbox_heading_delete_icon_title');?>' style='cursor:pointer;' onclick='return delete_pmsg(" + id + ");' src='<?php echo base_url();?>assets/images/delete_icon.png'></a></a>");
				open_reply();
			}
		}
	}
		   ...
		   alert("<?php echo $this->lang->line('inbox_please_select_message_which_you_want_to_delete');?>");
			...
			if (arrInputId != "") {
			   $.ajax({
				   type: "GET",
				   url: urlCustom,
				   data:  'datas='+arrInputId+'&type=from',
				   success: function(msg) {
						//alert(msg);
						//$("#inboxes_del_str").val() = "";
						document.getElementById("inboxes_del_str").value = "";
						location.href = "<?php echo base_url().'property/get_message'; ?>"+pageString;
				   },
				   error: function() {
					   alert("<?php echo $this->lang->line('inbox_there_is_something_wrong');?>");
				   }
