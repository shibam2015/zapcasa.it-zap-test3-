	...
	if(property_name==''){
		...
		$.ajax({
			...
			data:{
				...
				furnished : $("select[name='furnished']").val(),
				roommates : $("select[name='roommates']").val(),
				occupation : $("select[name='occupation']").val(),
				smokers : ($("input[name='smokers']").is(':checked'))?1:0,
				pets : ($("input[name='pets']").is(':checked'))?1:0,
				elevator : ($("input[name='elevator']").is(':checked'))?1:0,
				air_conditioning : ($("input[name='air_conditioning']").is(':checked'))?1:0,
				garden : ($("input[name='garden']").is(':checked'))?1:0,
				terrace : ($("input[name='terrace']").is(':checked'))?1:0,
				balcony : ($("input[name='balcony']").is(':checked'))?1:0,
					...
					<div id="login-content2" class="dp-content">
						<div class="basic_div_part" style="padding:0;">
							<fieldset id="inputs">
								...
								<input type="text" placeholder="<?php echo $this->lang->line('search_header_any_price_min');?>" name="min_price" id="min_price" value="<?php echo (isset($min_price)) ? $min_price : ''; ?>" class="groupOfCurrencyBox">
								<p style="margin:0 0 3px 0"><?php echo $this->lang->line('search_header_any_price_max');?></p>
								<input type="text" placeholder="<?php echo $this->lang->line('search_header_any_price_max');?>" name="max_price" id="max_price" value="<?php echo (isset($max_price)) ? $max_price : ''; ?>" class="groupOfCurrencyBox">
							</fieldset>
						</div>
					</div>
				</li>
