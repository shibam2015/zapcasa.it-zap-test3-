...
class propertym extends CI_Model {
    ...
    public function getProeprtiesByFilter($filters = '', $startpoint, $perpage)
    {
        ...
        $featured_property = get_all_featured_property($where);
        //$featured_property_count = count($featured_property);
        if (count($featured_property) > 0) {
            foreach ($featured_property as $property_featured_detail) {
                $featuredProperties_all[] = $property_featured_detail['property_id'];
            }
    ...
    public function count_get_saved_property()
    {
        $uid = $this->session->userdata('user_id');
        $sql = "SELECT sp.*, zc_city.*, zc_provience.* FROM zc_saved_property as sp JOIN zc_property_details as pd ON sp.property_id = pd.property_id ";
        $sql .= "JOIN zc_user as u ON pd.property_post_by = u.user_id left join zc_city on pd.city = zc_city.city_id left join zc_provience on pd.provience = zc_provience.provience_id WHERE sp.saved_by_user_id = " . $uid . " AND pd.property_approval='1'  AND pd.property_status = '2'  AND u.verified='1'  ORDER BY sp.saved_date";
    ...
    public function get_similar_property($property_id)
    {
        ...
        if (count($rs) > 0) {
            ...
            $sql_search_similar = "select a.* from zc_property_details a LEFT JOIN zc_user b on a.property_post_by = b.user_id where b.status = '1' and b.verified = '1' and a.contract_id='" . $rs[0]['contract_id'] . "' and a.typology='" . $rs[0]['typology'] . "' and a.city='" . $rs[0]['city'] . "' and a.provience='" . $rs[0]['provience'] . "' and a.property_id!='" . $property_id . "' AND a.property_status='2' AND a.suspention_status <> '1' and property_approval ='1'  ORDER BY a.price DESC LIMIT 0 , 5 ";
