...
class NearByProperty extends CI_Controller {
	...
	public function index() {
		...
		$data['property_details']=$this->Common_model->get_all_property($config, $this->uri->segment($config['uri_segment'], 0));
	...
	public function index_edit($prp_id = '') {
			...
			if( $nearbypropertyDetailsId > 0 ) {	
				if( isset($_FILES['property_image']['name']) && ($_FILES['property_image']['name'] != "" )) {
					...
					if (in_array($name_type, $valid_array_type)) {
						...
						@unlink($thumb_path);
						copy($_FILES['property_image']['tmp_name'], $new_thumb_targetpath);
		...
		$data['provience_list']=$this->Common_model->get_provience_list($_COOKIE['language']);
	...
	public function index_edit_page( $cat_id = 0  ) {
		...
		if( isset($_REQUEST['Submit']) && ($_REQUEST['Submit'] == "Submit" ) && ($_REQUEST['nearbypropertyId'] != 0 ) ) {
			...
			$dataStr = " `category_id` ='".$_REQUEST['category_id']."',`name`='".$_REQUEST['name']."',`opt_name` ='".$_REQUEST['opt_name']."',`provience`='".$_REQUEST['province']."',`city` ='".$_REQUEST['city']."',`zip`='".$_REQUEST['zip']."',`street_address` ='".$_REQUEST['street_address']."',`street_no`='".$_REQUEST['street_no']."',`latitude` ='".$latitude."',`longitude`='".$longitude."',`modified_on`='".time()."'";
		...
		if( count( $data['result'] ) > 0 ) {
			...
			redirect('NearByProperty/index');
		}
	}
	
	public function statuschange_prop( $cat_id = 0 ) {
		$this->load->model('Common_model');
		$data = array();
		if( isset( $cat_id ) && ( $cat_id != 0) ) {
			$attr = "property_details_id";
	...
	function upload_image_1($form_field_name,$property_id) {
		...
		$structure = './assets/uploads/NearByProperty/'.$new_file;
		if (!is_dir('assets/uploads/NearByProperty/'.$new_file)) {
			mkdir('./assets/uploads/NearByProperty/'.$new_file, 0777, true);
